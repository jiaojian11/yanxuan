...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-10" style="margin-left: 30px;margin-top: 20px;">
     ...
     促销名称:  
        <input value="{{$fix->l_name}}" disabled="true" ><br><br>
      优惠时间:<input type="text" name="cctime" value="{{date('Y-m-d H:i:s',$fix->l_ctime)}}" disabled="true" >  -  
      <input type="text" name="aatime" value="{{date('Y-m-d H:i:s',$fix->l_ltime)}}" disabled="true" ><br><br>
      优惠力度: <input type="text" value="{{$fix->l_price}}" disabled="true"><br><br>
      <input type="hidden" name="lid" value="{{$fix->l_id}}">
      <a href="{{url('admin/promotion/actadd')}}?lid={{$fix->l_id}}" ><button type="">继续添加</button></a>
      <form action="/admin/promotion/search" method="get">
       <div class="row">
        <div class="col-sm-12">
          <span style="margin-left: 300px;font-size:20px;">提交以后不能修改(只能删除或添加到提交)</span><br><br>
 ...
 -->            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">加入优惠</th>
          ...
          @if($v->g_yhid==$fix->l_id)
           ...
           <!--  <td>1</td> 
            ...
            <td class="center">
              <button type="button" class="btn btn-primary btn-xs abc" aid="{{$v->g_id}}" lid="{{$fix->l_id}}">取消优惠</button>
            </td> 
           </tr>
           @endif
           @endforeach
      ...
      <a href="{{url('/admin/promotion/alllist')}}"><button>确定</button></a>
    </div>
    </div>
    <!-- =========================================================================================== -->
  </div>
...
<!-- 引入js文件 -->
  ...
  <script>
    $('.abc').click(function(){
      ...
      // 发送ajax
      $.post('/admin/promotion/qxiao',{id:inp,lid:inc},function(data){
        if(data==1){
          ins.html('加入优惠');
        }else if(data==0){
          ins.html('取消优惠');
