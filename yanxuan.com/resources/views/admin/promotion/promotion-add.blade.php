...
<div class="wrapper">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="{{url('admin/promotion/rules')}}"><i class="fa fa-circle-o"></i> 营销规则</a></li>
  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-10" style="margin-left: 30px;margin-top: 20px;">
      ...
      <form action="/admin/promotion/search" method="get">
       <div class="row">
        <div class="col-sm-6">
        <div style="float: left;margin-top: 1px;">
           <input type="radio" name="check" checked>
        </div>
        <span style="float: left;">限时优惠</span><br><br>
        促销名称:  
           <input  name="lname" value="{{$fix->l_name}}" readonly ><br><br>
         优惠时间:<input type="text" name="cctime" value="{{date('Y-m-d H:i:s',$fix->l_ctime)}}" readonly >  -  
         <input type="text" name="aatime" value="{{date('Y-m-d H:i:s',$fix->l_ltime)}}" readonly ><br><br>
         优惠力度: <input type="text" name="ld" value="{{$fix->l_price}}" readonly><br><br>
         <input type="hidden" name="lid" value="{{$fix->l_id}}">
         <div id="dataTables-example_filter" class="dataTables_filter">
          商品搜索:  <input name="uname" aria-controls="dataTables-example" type="text" style="width: 150px;height:33px;margin-top: 2px;" />
      ...
      <div class="row">
        <div class="col-sm-12">
          <span style="margin-left: 300px;font-size:20px;">每次优惠添加商品不要超过4件</span><br><br>
         <table class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info"> 
          <thead> 
           <tr role="row">
            <!-- <th class="sorting_asc" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 191px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">选择</th> -->
            <!-- <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 220px;" aria-label="Browser: activate to sort column ascending"></th> -->
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 200px;" aria-label="Platform(s): activate to sort column ascending">商品名称</th>
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 164px;" aria-label="Engine version: activate to sort column ascending">商品图片</th>
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">商品简介</th>
           <!--  <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">创建时间</th>
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">最后登陆时间</th>
 -->        <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">加入优惠</th>
           </tr> 
          </thead> 
          <tbody>
          ...
          <tr class="gradeA odd" role="row"> 
              <td colspan="5" align="center">暂无商品!</td>
           </tr>
           @else
          @foreach($data as $k=>$v)
          @if($v->g_yhid==0)
           <tr class="gradeA odd" role="row"> 
            <!-- <td class="sorting_1"><input type="checkbox" name="id" value="{{$v->g_id}}"></td>  -->
            <!-- <td class="center"></td>  -->
           <!--  <td>1</td> 
            <td class="center">1</td> 
            <td class="center">1</td>  -->

            <td class="center" style="width:200px;">{{$v->g_name}}</td> 
            <td class="center" style="width:200px;"><img src="{{explode(',',$v->g_pic)[0]}}" width="50px" height="50px"></td>  
            <td class="center" style="width:200px;">{{$v->g_intro}}</td> 
            <td class="center">
              <button type="button" class="btn btn-primary btn-xs abc" aid="{{$v->g_id}}" lid="{{$fix->l_id}}">加入优惠</button>
            </td> 
           </tr>
            @endif
           @endforeach
      ...
      @if(!empty($data[0]))
        {!! $data->appends($a)->render() !!}
      @endif
      <div style="float:right;">
       <a href="{{url('admin/promotion/check')}}?lid={{$fix->l_id}}"><button>确定</button></a>
    </div>
...
<!-- 引入js文件 -->
  ...
  <script>
    $('.abc').click(function(){
      var inp = $(this).attr('aid');
      var inc = $(this).attr('lid');
      var ins = $(this);
      // alert(ins);
      // console.log(inp);
      $.ajaxSetup({
      ...
      // 发送ajax
      $.post('/admin/promotion/add',{id:inp,lid:inc},function(data){
        if(data==1){
          ins.html('取消优惠');
        }else if(data==0){
          ins.html('加入优惠');
