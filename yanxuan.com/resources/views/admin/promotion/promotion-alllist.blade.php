...
<div class="wrapper">
  ...
  <div class="content-wrapper" style="height: 1000px;background: #fff;">
    ...
    <!-- /.content -->
    <!-- =========================================================================================== -->
    <span class="logo-lg" style="margin-left: 400px;font-size:24px;color:red;"><b>注意:取消将删除该活动</b></span>
    @foreach($data as $k=>$v)

    <div class="row jiao" style="margin-top: 40px;">
    <div class="col-md-10" style="margin-left: 30px;">
     ...
     促销名称:  
        <input value="{{$v->l_name}}" disabled="true" ><br><br>
      优惠时间:<input type="text" name="cctime" value="{{date("Y-m-d H:i:s",$v->l_ctime)}}" disabled="true" >  -  
      <input type="text" name="aatime" value="{{date("Y-m-d H:i:s",$v->l_ltime)}}" disabled="true" ><br><br>
      优惠力度: <input type="text" value="{{$v->l_price}}" disabled="true"><br><br>
      
      <form action="/admin/promotion/del" method="get">
      <input type="hidden" value="{{$v->l_id}}" name="lid">
       <div class="row">
        <div class="col-sm-12">
         <table class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info"> 
          <thead> 
           <tr role="row">
            ...
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">商品价格</th>
            <th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 119px;" aria-label="CSS grade: activate to sort column ascending">优惠价格</th>
        </tr> 
          </thead> 
          <tbody>
            @foreach($arr[$k] as $a=>$b)
           <tr class="gradeA odd" role="row">  
            <td class="center" style="width:200px;">{{$b->g_name}}</td> 
            <td class="center" style="width:200px;"><img src="{{explode(',',$b->g_pic)[0]}}" width="50px" height="50px"></td>  
            <td class="center" style="width:200px;">{{$b->g_intro}}</td> 
            <td class="center" style="width:200px;">{{$b->g_price}}</td> 
            <td class="center" style="width:200px;">{{$b->g_price-$v->l_price}}</td>
            <td><input type="hidden" name="coupon" value="{{$b->g_price-$v->l_price}}"></td> 
           </tr>
           @endforeach
          </tbody> 
         </table>
       ...
       @if($v->l_shelves==0)
       <button type="button" class="abcd" bid="{{$v->l_id}}">提交</button>
       @endif
...
<script>
  $('.abcd').click(function(){
    ...
    $.post('/admin/promotion/do',{lid:bid,coupon:did},function(data){
      // alert(data);
      if(data){
        cid.remove();
      }
