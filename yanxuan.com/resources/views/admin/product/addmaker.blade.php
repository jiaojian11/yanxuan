     ...
     <ul class="sidebar-menu">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="/admin/product/addmaker"><i class="fa fa-circle-o"></i> 制造商管理</a></li>
...
<!-- ======================================================================================== -->
    ...
    <div class="row" style="margin-top: 30px;">
      <!-- 左边添加板块 -->
      <div class="col-xs-4">
        <form action="/admin/product/addmakers" method="post" enctype="multipart/form-data">
          <!-- 第一行 产品制造商编号-->
          <div class="row" style="margin-top: 30px;">
            <div class="col-xs-3" style="width: 135px;margin-left: 40px;">
              <div class="box box-warning" style="border-top-color: #3c8dbc;height: 34px;width: 120px;">
                <div class="box-header with-border">
                  <h3 class="box-title">制造商名称:</h3>
                </div>
              </div>
            </div>
            <div class="col-xs-3">
              <input class="form-control" placeholder="请输入制造商名称" type="text" name="m_name" style="width:200px;">
            </div>
          </div>
          <!-- 第二行 制造商图片-->
          <div class="row">
          ...
          <div class="row" >
            <div class="col-xs-3" style="width: 135px;margin-left: 40px;">
              <div class="box box-warning" style="border-top-color: #3c8dbc;height: 34px;width: 120px;">
                <div class="box-header with-border">
                  <h3 class="box-title">制造商简介:</h3>
                </div>
              </div>
            </div>
            <div class="col-xs-3">
              <textarea rows="5" cols="25" placeholder="请输入制造商简介" name="m_info"></textarea>
            </div>
          </div>
          {{ csrf_field() }}
          <button type="reset" class="btn btn-primary" style="margin-left: 130px;margin-top: 10px;">重置</button>
          <button id="tian" type="submit" class="btn btn-primary" style="margin-left: 20px;margin-top: 10px;">添加</button>
        </form>
      </div>
      <!-- 右侧制造商详情表 -->
      <div class="col-xs-3" style="margin:30px;">
        <div class="box" style="float:left;height:700px;width: 700px;margin-left: -15px;border-top-color:#3c8dbc;position:absolute;">
          <div class="box-header">
            <center>
              <h3 class="box-title">制造商管理</h3>
            </center>
          </div>
          <div class="box-body" style="width: 420px;margin-left: 30px;">
            <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
              <form action="/admin/product/addmaker" method="get">
              <div class="row">
                <div id="dataTables-example_filter" class="dataTables_filter" style="margin-left:340px;width: 450px;">
                  <label>制造商名称:<input type="search" name="keyword" class="form-control input-sm" placeholder="" aria-controls="dataTables-example" /></label>
                    <button class="btn btn-primary">搜索</button>
                 </div>
              </div>
              </form>
              <div class="row" style="margin-top: 10px;">
                <div class="col-sm-12" style="width:700px;margin-left: -30px;">
                  <table class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info" >
                    <thead>
                      <tr role="row">
                        <th style="width:90px;" class="sorting_desc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Rendering engine: activate to sort column ascending" aria-sort="descending">
                            <center>制造商编号</center>
                        </th>
                        <th style="width:90px;" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending"> 
                            <center>制造商名称</center>
                        </th>
                        <th style="width:100px;" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">
                            <center>制造商图片</center>
                        </th>
                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">
                            <center>制造商简介</center>
                        </th>
                        <th style="width:90px;" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">
                            <center> 操作</center>
                        </th>
                      </tr>
                    </thead>
                    ...
                    @foreach($datas as $k=>$v)
                        <tr role="row" class="odd">
                            ...
                            <td><input  type="text" name="" value="{{$v->m_name}}" disabled style="width:90px;background:#fff;border:0px;"></td>
                            <td>
                              <a href="#" type="button" class="umpic" mid="{{$v->m_id}}" data-toggle="modal" data-target="#myModal">
                                <image src="{{$v->m_pic}}" style="width: 90px;height: 70px;">
                              </a>
                            </td>
                            <td><textarea  name="" cols="35" rows="4" disabled style="background:#fff;border:0px;">{{$v->m_info}}</textarea></td>
                            <td>
                              <a href="" class="emaker" eid="{{$v->m_id}}">修改</a>
                              <a href="/admin/product/delmaker?id={{$v->m_id}}">删除</a>
                            </td>
                        </tr>
                    @endforeach 
                    </tbody>
                ...
                <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                  <form action="/admin/product/upmpic" method="post" enctype="multipart/form-data">
                  <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">修改制造商图片</h4>
                      </div>
  ...
  <!-- 一点击图片就获取该图片所属制造商的id 并将id传递给模态框里的隐藏input框，以便知道要修改的是哪个制造商的图片 -->
  <script type="text/javascript">
  ...
  <script type="text/javascript">
      ...
      //给所有的修改绑定单击事件 点击修改变成保存，input变成可修改
      $('.emaker').live('click',function(){
        var input=$(this).parents('tr').children('td:eq(1)').children('input');
        input.removeAttr('disabled');
        var textarea=$(this).parents('tr').children('td:eq(3)').children('textarea');
        textarea.removeAttr('disabled');
        $(this).html('保存');
        $(this).attr('class','maker');
        return false;
     })

      // 给保存绑定单击事件，点击提示修改是否成功
      $('.maker').live('click',function(){
        var links=$(this);
        var inputs=$(this).parents('tr').children('td:eq(1)').children('input');
        var textareas=$(this).parents('tr').children('td:eq(3)').children('textarea');
        var m_id=$(this).attr('eid');
        var m_name=inputs.val();
        var m_info=textareas.val();

        // 发送ajax
        $.get('/admin/product/updatem',{m_id:m_id,m_name:m_name,m_info:m_info},function(data){
          if(data==1){
              links.html('修改');
              inputs.attr('disabled','disabled');
              textareas.attr('disabled','disabled');
              links.attr('class','emaker');
              //获取提醒信息
              ...
              textareas.attr('disabled','disabled');
              links.attr('class','emaker');
              $('#successMessage').text('修改失败').fadeIn(1000);
              setTimeout(function(){
                $('#successMessage').fadeOut(1000);
              },2000);
          }
