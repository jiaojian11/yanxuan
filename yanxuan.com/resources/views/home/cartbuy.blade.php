...
@section('contents')
	   ...
	   <div class="g-row"> 
	    <div style="position: absolute; right: 0px; top: -36px; display: block;" class="m-noticeList noticeList j-topNoticeList limitLength"> 
	     <ul class="j-list list"> 
	      <li class="j-notice notice"> <a target="_blank" href="" class="txt f-text-underline"> 1周年庆：严选开启支持港澳台配送测试 </a> </li> 
	     </ul> 
	    </div> 
	    <div class="g-panel"> 
	     <div class="w-panel"> 
	     ...
	     <div class="m-table"> 
	      ...
	      <table class="m-orderTable"> 
	       ...
	       <tbody id="j-items" class="tbody">
	        ...
	        <tr class="tr">
	         <td class="td col1"><img style="margin-top:-30px" src="{{explode(',',$value->g_pic)[0]}}" class="img" /></td>
	         <td class="td col2">
	          <div class="pd-info" style="margin-bottom:40px;position:relative;top:60px">
	           ...
	           <p class="line line-1"><span>{{$value->gd_nature}} {{$value->gd_color}} {{$value->gd_size}}</span></p>
	          </div>
	          </td>
				@if($value->g_yhid>0)

				@if($llltime[$key]=='')
					<td class="td col3"><p>&yen;{{$value->g_price}}.00</p></td>
					<td class="td col4">{{$value->carg_num}}</td>
					<td class="td col5">&yen;{{$value->g_price * $value->carg_num}}.00</td>
					<td class="td col6 ">&yen;<span class="shen">{{$value->g_price * $value->carg_num}}.00</span></td>
					@elseif($llltime[$key]->l_ctime<time()&&$llltime[$key]->l_ltime>=time())

					<td class="td col3"><p>&yen;{{$value->g_coupon}}.00</p>
					<p style="text-decoration:line-through;color:#999">¥{{$value->g_price}}.00</p>
					<span class="u-promotion-tag">限时购</span>
					</td>
					<td class="td col4">{{$value->carg_num}}</td>
					<td class="td col5">&yen;{{($value->g_coupon) * $value->carg_num}}.00</td>
					<td class="td col6 ">&yen;<span class="shen">{{($value->g_coupon) * $value->carg_num }}.00</span></td>
					@elseif($llltime[$key]->l_ctime>time())
					 <td class="td col3"><p>&yen;{{$value->g_price}}.00</p></td>
					 <td class="td col4">{{$value->carg_num}}</td>
					 <td class="td col5">&yen;{{$value->g_price * $value->carg_num}}.00</td>
					 <td class="td col6 ">&yen;<span class="shen">{{$value->g_price * $value->carg_num}}.00</span></td>
	...
	<form action="/home/pay" method="post" >
	       ...
	       <div class="line5">
	        ...
	        <input type="hidden" name="ud_id" value="" id="pos1">
	        <input type="hidden" name="price_shifu" value="" id="pos2">
	        <input type="hidden" name="yunfei" value="" id="pos3">
	        <input type="hidden" name="heji" value="" id="pos4">
	        <input type="hidden" name="_token" value="{{ csrf_token() }}">
	        <div class="notice">
	   ...
	   <!-- 模态框在此开始 -->
	            ...
	            @if($v->c_status == 1)
				 ...
				 @endif 
	               ...
	               <div class="inner"> 
	                ...
	                <div class="range"> 
	                 <span class="text"> 商品总金额满<span class="man" gid="{{$v->c_id}}">{{$v->c_claim}}</span>元  减<span class="jian">{{$v->c_money}}</span>元</span> 
	     ...
	     <!-- 切换地址的模态框在此 -->
	      ...
	      <div class="m-overlay m-overlay-ani qiehuan" style="z-index: 1350;display:none"> 
	        <div class="w-mask w-mask-ani j-mask f-ani-mask"></div> 
	        <div class="m-pop f-scroll-y overlay-container-ani f-tlbr j-overlay-container m-pop-changeAddr f-ani-bouncein" style="top: 167.5px; display: block;"> 
...
@section('js')
	...
	$(function(){
		...
		$('#j-couponDialogOK').click(function(){
			...
			gid = $('#hao').children('.inner').children('.range').children('.text').children('.man').attr('gid');
			// console.log(youman);
